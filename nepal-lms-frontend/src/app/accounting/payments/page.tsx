...
export default async function AccountingPaymentsPage({ searchParams }: { searchParams: PageSearchParams }) {
  ...
  const filtered = items
    .filter((item) => matchesQuery(q, item.id, item.student, item.course, item.method, item.status, item.risk) && (!status || item.status.toLocaleLowerCase() === status.toLocaleLowerCase()) && (!method || item.method.toLocaleLowerCase().includes(method.toLocaleLowerCase())))
    ...
    <>
      ...
      <Panel className="mt-6">
        <ListFilters
          ...
          resetHref={base}
          fields={[
            { name: "status", label: "Payment status", value: status, options: [{ value: "", label: "All statuses" }, { value: "Submitted", label: "Submitted" }, { value: "Under review", label: "Under review" }, { value: "Approved", label: "Approved" }, { value: "Rejected", label: "Rejected" }, { value: "Refunded", label: "Refunded" }] },
