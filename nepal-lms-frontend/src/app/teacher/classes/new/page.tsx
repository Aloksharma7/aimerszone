...
export default async function NewTeacherClassPage() {
  ...
  return <><PageHeader back={{ href: "/teacher/classes", label: "All classes" }} eyebrow="Schedule" title="Create class session" description="Schedule a class only for a batch assigned to your teacher account." /><div className="grid gap-6 xl:grid-cols-[1fr_340px]"><Panel><TeacherSessionForm batches={batches} /></Panel><aside className="space-y-5"><AlertBox title="Laravel remains authoritative" tone="info">The API rechecks batch assignment, schedule conflicts, Zoom configuration and teacher permission before creating a session.</AlertBox><Panel><CalendarDays className="h-6 w-6 text-brand-700" /><h2 className="mt-4 font-bold text-slate-950">Time zone</h2><p className="mt-2 text-sm leading-6 text-slate-600">Enter the schedule in Nepal Time. Laravel stores an unambiguous timestamp and returns display-ready times.</p></Panel></aside></div></>;
}
