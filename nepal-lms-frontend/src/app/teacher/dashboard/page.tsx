...
export default async function TeacherDashboardPage() {
  ...
  return (
    <>
      ...
      {next ? <LiveClassCard title={next.title} course={`${next.course} · ${next.batch}`} teacher={`${next.students} enrolled students`} time={`${next.date} · ${next.time}`} href={`/teacher/classes/${next.id}`} teacherMode sessionId={next.id} startAvailable={Boolean(next.canStart && next.startAvailable)} /> : <Panel><p className="font-semibold text-slate-900">No class is currently scheduled.</p><p className="mt-1 text-sm text-slate-500">New sessions will appear here once you have an assigned schedule.</p></Panel>}
