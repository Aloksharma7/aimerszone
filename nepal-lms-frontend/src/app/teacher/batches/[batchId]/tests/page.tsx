...
export default async function TeacherTestsPage({ params, searchParams }: { params: Promise<{ batchId: string }>; searchParams: PageSearchParams }) {
  ...
  const [detail, tests] = await Promise.all([getTeacherBatch(batchId), getTeacherTests(batchId)]);
  if (!detail) notFound();
  ...
  return (
    <>
      <PageHeader eyebrow="Assessment" title="Tests" description={`${detail.batch.course} · ${detail.batch.batch}`} actions={<ButtonLink href={`/teacher/tests/new?batchId=${encodeURIComponent(batchId)}`}><Plus className="h-4 w-4" />Create test</ButtonLink>} />
      <Panel>
