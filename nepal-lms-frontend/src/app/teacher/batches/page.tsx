...
import { matchesQuery, searchTerm, type PageSearchParams } from "@/lib/search-params";

export default async function TeacherBatchesPage({ searchParams }: { searchParams: PageSearchParams }) {
  const raw = await searchParams;

  // The header search box targets this page. It previously accepted no
  // parameters at all, so typing a batch name here did nothing.
  const query = searchTerm(raw);
  const all = await getTeacherBatches();
  const batches = all.filter((batch) => matchesQuery(query, batch.course, batch.batch, batch.schedule, batch.status));
