...
export default async function StaffCoursesPage({ searchParams }: { searchParams: PageSearchParams }) {
  ...
  const rows = items.map((course) => ({ id: course.id || course.slug, title: course.title, code: course.code, category: course.category, price: course.isFree ? "Free" : formatNpr(course.price), batches: course.batchCount ?? 0, enrollments: course.enrollments ?? 0, status: course.published ? "Published" : "Draft", href: `/staff/courses/${encodeURIComponent(course.id || course.slug)}` }));
  return (
    <>
