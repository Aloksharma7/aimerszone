...
export default async function NewPaymentPage({ searchParams }: { searchParams: Promise<{ course?: string; batch?: string }> }) {
  ...
  return <><PageHeader back={{ href: "/student/payments", label: "All payments" }} eyebrow="New payment" title="Submit payment proof" description="Follow the steps carefully. Laravel resolves the selected batch, expected amount and available payment methods." /><PaymentWizard courses={courses} initialCourseSlug={course} initialBatchId={batch} /></>;
}
