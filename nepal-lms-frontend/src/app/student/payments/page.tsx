...
import { getStudentPayments, getStudentPaymentsPage } from "@/lib/data/student";
import { buildQueryString, pageParam, type PageSearchParams } from "@/lib/search-params";
import { formatNpr } from "@/lib/utils";

export default async function StudentPaymentsPage({ searchParams }: { searchParams: PageSearchParams }) {
  const raw = await searchParams;
  const page = pageParam(raw);
  const [payments, { items, meta }] = await Promise.all([getStudentPayments(), getStudentPaymentsPage({ page })]);
  ...
  return (
    <>
      ...
      <Panel className="mt-6 p-0" padded={false}>{items.length ? <div className="divide-y divide-slate-100">{items.map((payment) => <Link key={payment.id} href={`/student/payments/${payment.id}`} className="grid gap-3 p-5 transition hover:bg-slate-50 sm:grid-cols-[1fr_auto] sm:items-center"><div><div className="flex flex-wrap items-center gap-2"><h2 className="font-bold text-slate-950">{payment.course}</h2><StatusBadge status={payment.status} /></div><p className="mt-1 text-sm text-slate-500">{payment.batch}</p><p className="mt-2 text-xs text-slate-400">{payment.id} · {payment.method} · Submitted {payment.submitted}</p>{payment.reason ? <p className="mt-2 text-sm text-red-700">{payment.reason}</p> : null}</div><div className="text-left sm:text-right"><p className="text-lg font-bold text-slate-950">{formatNpr(payment.amount)}</p><p className="mt-1 text-xs font-semibold text-brand-700">View details →</p></div></Link>)}</div> : <div className="p-5"><EmptyState title="No payment submissions" description="New payment proofs and their review status will appear here." action={<ButtonLink href="/student/payments/new">Submit payment proof</ButtonLink>} /></div>}</Panel>
      <Pagination meta={meta} buildHref={(target) => `/student/payments${buildQueryString({ page: String(target) })}`} />
    </>
  );
}
