    ...
    <>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_330px]">
        <div>
          ...
          <Panel className="mt-5"><p className="text-sm font-bold uppercase tracking-wider text-brand-700">{recording.module}</p><h1 className="mt-2 text-2xl font-bold text-slate-950">{recording.title}</h1><p className="mt-2 text-sm text-slate-500">{recording.teacher} · {recording.date} · {recording.duration}</p><div className="mt-5"><ProgressBar value={recording.progress} label="Watching progress" /></div><div className="mt-5 flex gap-3 rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm leading-6 text-blue-900"><Info className="mt-0.5 h-5 w-5 shrink-0" />Playback is loaded only after Laravel confirms this enrollment is active and the recording is released.</div></Panel>
