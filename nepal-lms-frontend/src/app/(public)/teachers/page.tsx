      ...
      <section className="bg-canvas py-12 sm:py-16">
        <div className="mx-auto grid max-w-7xl gap-6 px-4 sm:grid-cols-2 sm:px-6 lg:grid-cols-3 lg:px-8">
          {teachers.map((teacher) => (
            <Link key={teacher.slug} href={`/teachers/${teacher.slug}`} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white transition hover:-translate-y-0.5 hover:border-blue-200 hover:shadow-card">
              <div className="relative">
                {teacher.avatarUrl ? (
                  // eslint-disable-next-line @next/next/no-img-element -- admin-uploaded asset, not a Next-optimized asset
                  <img src={teacher.avatarUrl} alt="" className="aspect-4/3 w-full object-cover" />
                ) : (
                  <div className={`flex aspect-4/3 w-full items-center justify-center bg-linear-to-br ${teacher.accent} text-6xl font-bold text-white`}>{teacher.initials}</div>
                )}
                <Badge tone="blue" className="absolute right-3 top-3">Faculty</Badge>
              </div>
              <div className="p-6">
                <h2 className="text-xl font-bold text-slate-950 group-hover:text-brand-700">{teacher.name}</h2>
                <p className="mt-1 font-semibold text-brand-700">{teacher.role}</p>
                <p className="mt-4 text-sm leading-7 text-slate-600">{teacher.bio}</p>
                <div className="mt-5 flex flex-wrap gap-2">{teacher.subjects.map((subject) => <Badge key={subject}>{subject}</Badge>)}</div>
                <p className="mt-5 flex items-center gap-2 text-sm font-bold text-brand-700">View profile<ArrowRight className="h-4 w-4" /></p>
              </div>
            </Link>
          ))}
        </div>
