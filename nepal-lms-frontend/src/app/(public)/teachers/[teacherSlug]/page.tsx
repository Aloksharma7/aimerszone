...
export default async function TeacherProfilePage({ params }: { params: Promise<{ teacherSlug: string }> }) {
  ...
  return (
    <><section className="border-b border-slate-200 bg-canvas py-12"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><Link href="/teachers" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-600 hover:text-brand-700"><ArrowLeft className="h-4 w-4" />All teachers</Link><div className="mt-7 grid gap-7 lg:grid-cols-[auto_1fr] lg:items-center">{teacher.avatarUrl ? (
        ...
        <img src={teacher.avatarUrl} alt="" className="h-48 w-48 rounded-3xl object-cover shadow-card" />
      ) : (
        <div className={`flex h-48 w-48 items-center justify-center rounded-3xl bg-linear-to-br ${teacher.accent} text-5xl font-bold text-white shadow-card`}>{teacher.initials}</div>
