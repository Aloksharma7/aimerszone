 ...
 */
export function EnrollmentRequestDecision({ requestId }: { requestId: string }) {
  const router = useRouter();
  const mockMode = isMockDataEnabled();
  ...
  async function decide(decision: "approve" | "reject", reason: string) {
    ...
    await browserRequest({
      url: `/api/v1/admin/enrollment-requests/${encodeURIComponent(requestId)}/decision`,
      method: "POST",
      data: { decision, reason },
      headers: { "Idempotency-Key": createIdempotencyKey("enrollment-decision") },
    });
  ...
  return (
    <div className="flex flex-wrap gap-2">
      <ReasonAction
        label="Approve"
        title="Approve and grant the seat?"
        description="Waiving a fee on a paid course requires Super Admin approval and is reviewed against the payment record. If the student actually paid, record that payment instead of approving here."
        confirmLabel="Approve and grant the seat"
        tone="primary"
      ...
      <ReasonAction
        ...
        description="The requesting officer sees your reason. No seat is granted."
