...
export function UserProfileEditor({ userId, user }: { userId: string; user: AdminUser & { studentCode?: string | null } }) {
  ...
  async function save() {
    ...
    try {
      ...
      setNotice({ tone: "success", title: mockMode ? "Preview validated" : "Profile saved", message: mockMode ? "The form passed validation. Preview mode does not save changes." : "Verified profile information was updated and audited." });
...
export function AccountControlPanel({ userId, initialStatus }: { userId: string; initialStatus: string }) {
  ...
  async function act(action: "password-reset" | "mfa-reset" | "revoke-sessions" | "suspend" | "reactivate") {
    ...
    try {
      ...
      setNotice({ tone: "success", title: mockMode ? "Preview validated" : "Account action completed", message: mockMode ? "The form passed validation. Preview mode does not perform the action." : "The server accepted the request and recorded the audit reason." });
      setReason("");
      if (!mockMode) router.refresh();
